...
namespace gol { 
      ...
      std::string filename) 
  {
    try {
      ...
      throw data_exception( 
        std::string("load_public_transport(): ") 
        +  e.what());
    }
  }

  // Puts a serialized object in the cache with key filename
  void save_public_transport(
      const timetable_Rt& tt, 
      std::string filename) 
  {
  ...
  { 
    ...
    if ((sys_path(filename).extension()) == ".json") {
      logger(logINFO) 
        << left("[cache]", 14) 
        << "Parsing Test JSON Timetable";
      //osm::json_director<raptor_timetable_builder> director(
        //filename.c_str(), builder);
    ...
    //} 
      ...
      throw data_exception( 
        std::string("parse_public_transport(): File extension unknown ") 
    ...
    std::string model) 
  {
    ...
    { 
      ...
      sys_path serialized = 
        root / 
        sys_path("cache") / 
        sys_path((std::string(filename).append(".")
                                       .append(model))
                                       .append(".ttsrz"))
          .filename();
      ...
      logger(logERROR) 
        ...
        << e.what();
      throw data_exception( 
        std::string("retrieve_public_transport(): ") + e.what());  
    }
  } 
