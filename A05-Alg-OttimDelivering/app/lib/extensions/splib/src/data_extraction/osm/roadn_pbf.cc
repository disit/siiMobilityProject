      ...
      { 
               ...
               _relation_restriction.end() )
        {
            std::vector< std::pair<
               ...
               std::vector<std::string>  
            > > turn_res = { std::make_pair(way_from, way_to) };
...
void roadn_pbf_parser<BuilderT>::construct_model() 
{
  ...
  for (auto VW : _relation_restriction)
  {
     std::string              via = VW.first;
     std::vector<std::string> from, to;
  
     for (auto FT : VW.second)
     {
       for(auto wayId : FT.first) 
         for (auto nId : _restricted_way[wayId])
           from.push_back(nId);
       for(auto wayId : FT.second) 
         for (auto nId : _restricted_way[wayId])
           to.push_back(nId);

       _builder-> add_restriction(
           via, from, to);           
     }
