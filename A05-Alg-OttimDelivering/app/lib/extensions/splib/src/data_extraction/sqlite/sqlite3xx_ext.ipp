...
namespace sqlite3xx
{
  namespace ext
  {
    ...
    namespace
    {
      ...
      {
        auto f = static_cast<
          function::function_handler*>(sqlite3_user_data(ctx));
      ...
      void step_impl(sqlite3_context* ctx, int nargs, sqlite3_value** values)
      {
        auto p = static_cast<
          std::pair<
            aggregate::pfunction_base, 
            aggregate::pfunction_base>*>(sqlite3_user_data(ctx));
    ...
    {
      sqlite3_result_text(
        ctx_, value, std::strlen(value), fcopy ? SQLITE_TRANSIENT : SQLITE_STATIC);
    }

    inline void context::result(void const* value, int n, bool fcopy)
    {
      sqlite3_result_blob(
        ctx_, value, n, fcopy ? SQLITE_TRANSIENT : SQLITE_STATIC );
    }
    ...
    inline int function::create(char const* name, function_handler h, int nargs)
    {
      ...
      return sqlite3_create_function(
        db_, name, nargs, SQLITE_UTF8, fh_[name].get(), function_impl, 0, 0);
    }
    ...
    inline int aggregate::create(
      char const*      name, 
      function_handler s, 
      function_handler f, 
      int              nargs)
    {
      ah_[name] = std::make_pair(
        pfunction_base(
          new function_handler(s)), pfunction_base(new function_handler(f)));
      return sqlite3_create_function(
        db_, name, nargs, SQLITE_UTF8, &ah_[name], 0, step_impl, finalize_impl);
    }
